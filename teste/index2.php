...
if (isset($_GET['id_contr'])) {
    $ids = (int) $_GET['id_contr'];
}
if (isset($_POST['id_contr'])) {
    $ids = (int) $_POST['id_contr'];
}
...
$sqcon = "SELECT * FROM contrato WHERE id_contrato = '$ids'";
...
$result_events = "SELECT id, title, color, start, end, observacao FROM events WHERE id_contr = '$ids' ";
$resultado_events = mysqli_query($conection, $result_events)or die('Não foi possivel conectar ao MySQL');
?>
<!DOCTYPE html>
<html lang="pt-br">
    <head>
        ...
        <script>
            $(document).ready(function () {
                $('#calendar').fullCalendar({
                    ...
                    events: [
<?php
while ($row_envents = mysqli_fetch_array($resultado_events)) {
    ?>
                            {
                                id: '<?php echo $row_envents ['id']; ?>',
                                title: '<?php echo $row_envents ['title']; ?>',
                                color: '<?php echo $row_envents ['color']; ?>',
                                start: '<?php echo $row_envents ['start']; ?>',
                                end: '<?php echo $row_envents ['end']; ?>',
                                observacao: '<?php echo $row_envents ['observacao']; ?>',
                            }, <?php
}
?>
                    ]
    ...
    <body >
        <div class= "return">
            <a href="http://10.34.17.143/sp/idex.php?id=<?php echo $ids; ?>" class="btn btn-primary btn-lg btn-block"  ><?php echo "RG:" . $rg ?>   </a>
        </div>
        <div class="container ">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form">
                            <form class="form-horizontal" method="POST" action="proc_edit_evento.php">
                                ...
                                <input type="hidden" class="form-control" name="id_contr" id= "cid_contr" value="<?php echo $ids ?>">	
