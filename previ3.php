...
<div class=" col-12 justify-content-center "  >
    <form id='envia_msg' action='previ.php?<?php echo $id_contrato; ?>' method='POST' style="margin: auto;"> 
        <?php
        $sql1 = "SELECT * FROM  documentos WHERE categoria='$categoria_pesquisa'  AND id_contrato='$id_contrato' order by (categoria) limit $incio, $quantidade_pg";
    $resultado1 = mysqli_query($conection, $sql1)or die('Não foi possivel conectar ao MySQL');
            if (mysqli_num_rows($resultado1) == 0) {
                ...
                while ($registro1 = mysqli_fetch_array($resultado1)) {
                    ...
                    if ($categoria[$i] == 1) {
                        $doc = ' INICIAÇÃO DO CONTRATO';
                    } else if ($categoria[$i] == 2) {
                        $doc = 'ATA DE REUNIÃO';
                    } else if ($categoria[$i] == 3) {
                        $doc = 'GARANTIA DE EXECUÇÃO CONTRATUAL';
                    } else if ($categoria[$i] == 4) {
                        $doc = 'TERMO DE RECEBIMENTO PROVISÓRIO';
                    } else if ($categoria[$i] == 5) {
                        $doc = 'DECLARACAO DE ORIGENS DE BENS';
                    }else if ($categoria[$i] == 6) {
                        $doc = 'REQUISITOS TÉCNICOS PARA INSTALAÇÃO';
                    } else if ($categoria[$i] == 7) {
                        $doc = 'RELATÓRIO TECNICO';
                    } else if ($categoria[$i] == 8) {
                        $doc = 'RELATÓRIO PATRIMONIAL';
                    } else if ($categoria[$i] == 9) {
                        $doc = 'DECLARAÇÃODE ACEITE DE CAPACITAÇÃO';
                    }else if ($categoria[$i] == 10) {
                        $doc = 'TERMO DE RECEBIMENTO DEFINITIVO';
                    } else if ($categoria[$i] == 11) {
                        $doc = 'RELATÓRIO NE NIVEIS DE SERVIÇO';
                    } else if ($categoria[$i] == 12) {
                        $doc = 'RELATÓRIO PARAMETRIZADO DO CONTRATO';
                    } else if ($categoria[$i] == 13) {
                        $doc = 'DOCUMENTO DO FORNECEDOR';
                    }else if ($categoria[$i] == 14) {
                        $doc = 'SOLICITAÇÃO DO FORNECEDOR';
                    } else if ($categoria[$i] == 15) {
                        $doc = 'SOLICITAÇÃO INTERNA';
                    } else if ($categoria[$i] == 16) {
                        $doc = 'RESPOSTA A SOLICITAÇÃO INTERNA';
                    } else if ($categoria[$i] == 17) {
                        $doc = 'RESPOSTA AO FORNECEDOR';
                    }else if ($categoria[$i] == 18) {
                        $doc = 'TERMO DE APOSTILAMENTO';
                    } else if ($categoria[$i] == 19) {
                        $doc = 'TERMO DE ENCERRAMENTO DO CONTRATO';
                    } else if ($categoria[$i] == 20) {
                        $doc = 'REGISTRO DE LIÇÕES APRENDIDAS';
                    }
                     else if ($categoria[$i] == 21) {
                        $doc = 'CANAIS DE ATENDIMENTO';
                    }
                    ...
                    <div  style="border-style: solid;border-width: 1px;  border-color:#fff;padding: 15px;border-radius: 15px; background-color: #DDDDDD;">
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <input  type="text" style="font-size: 17px; font-weight: bold; color: blue;" class=" form-control" name="prazo"disabled value="<?php echo $doc . $regional[$i]?>"disabled>
                            </div>                                        
                            <div class="form-group col-md-4">
                                <label >Link</label>
